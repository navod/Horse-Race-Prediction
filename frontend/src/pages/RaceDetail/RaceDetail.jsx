...
const RaceDetail = () => {
  ...
  const fetchRaceDetails = async () => {
    setLoading(true);
    const race_details = await raceService
      .getRaceDetails(id)
      .then((resp) => {
        setRaceDetail(resp);
        setLoading(false);
            ...
            <div className="flex items-center gap-3 mt-10">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm text-black font-semibold">
                  {raceDetail.course} -{" "}
                  {moment(raceDetail.date).format("dddd DD MMMM YYYY, HH:mm")}
                </span>
              </div>
            </div>
          ...
          <div className="bg-[#F5F5F5] rounded-lg py-8 px-5 flex flex-col gap-4 mt-10">
            ...
            <div className="flex justify-between flex-col gap-2 xl:gap:0 lg:flex-row">
              <div className="flex gap-16 justify-between xl:justify-normal">
                ...
                <span className="text-sm ">{raceDetail.distance}</span>
              </div>

              <div className="flex gap-16 justify-between xl:justify-normal">
                <span className="text-sm font-bold">Prize</span>
                <span className="text-sm ">{raceDetail.prize}</span>
              </div>

              <div className="flex gap-16 justify-between xl:justify-normal">
                <span className="text-sm font-bold">Age</span>
                <span className="text-sm">{raceDetail.age}</span>
              </div>

              <div className="flex gap-16 justify-between xl:justify-normal">
                <span className="text-sm font-bold">finish_time</span>
                <span className="text-sm">
                  {raceDetail.finish_time ? raceDetail.finish_time : "-"}
                </span>
              </div>

              <div className="flex gap-16 justify-between xl:justify-normal">
                <span className="text-sm font-bold">finished</span>
                <span className="text-sm">{raceDetail.finished}</span>
              </div>
            </div>
          </div>
